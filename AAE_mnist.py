...
from tensorflow.keras.layers import Input, Dense, ReLU, Activation, Flatten
...
class AAE_mnist():
    def __init__(self,
        ...
        # optimizer
        self.ae_opt = Adam(0.0001)
        self.gen_opt = Adam(0.0001, beta_1=0, beta_2=0.9)
        self.disc_opt = Adam(0.0001, beta_1=0, beta_2=0.9)
        self.loss_object = BinaryCrossentropy(from_logits=True)
    
    def models(self, input_dim=(28, 28,1), dense_dim=1000, n_dim=2): # эта херня по ходу робит. 
        act = 'relu'     # 
        dec_dim = (100, 2)
        ...
        # encoder
        x = Flatten()(inputs)
        x = Dense(dense_dim, activation=act)(x)
        x = Dense(dense_dim, activation=act)(x)
        # code = Dense(n_dim, activation='linear')(x)
        code = Dense(n_dim)(x)

        # decoder
        input_encoded = Input(n_dim,)
        # input_encoded = Input(dec_dim,)
        x = Dense(dense_dim, activation=act)(input_encoded)
        x = Dense(dense_dim, activation=act)(x)
        x = Dense(28*28, activation='sigmoid')(x)
        out = tf.reshape(x, (-1, 28, 28, 1))

        # discriminator
        input_dsc = Input(n_dim,)
        x = Dense(dense_dim, activation=act)(input_dsc)
        x = Dense(dense_dim, activation=act)(x)
        # outd = Dense(1, activation='linear')(x)
        outd = Dense(1)(x)

        # models
        encoder = Model(inputs, code, name="encoder")
        decoder = Model(input_encoded, out, name="decoder")
        discriminator = Model(input_dsc, outd, name ="discriminator")
...
if __name__ == "__main__":
    AAE_mnist = AAE_mnist(n_dim=2,
                          ...
                          make_gif=True)
    # manner = Model()
    # a1 = np.zeros(2) # так чушь просто написал
    # a = manner(a1) # тут пишет что если субклассинг модель, то надо реализовать метод call
    # суть этой байды в том, что модель не в курсе того, что у нее на входе и выходе. ее содержимое не определено. 
    # решать это надо задав вход и наполнить ее содержимым ( слоями всякими ), потом 
    AAE_mnist.train()
